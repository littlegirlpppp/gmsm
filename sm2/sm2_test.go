...
func TestSm2(t *testing.T) {
	...
	ok, err := x509.WritePrivateKeytoPem("priv.pem", priv, nil) // 生成密钥文件
	if ok != true {
		log.Fatal(err)
	}
	pubKey, _ := priv.Public().(*PublicKey)
	ok, err = x509.WritePublicKeytoPem("pub.pem", pubKey, nil) // 生成公钥文件
	if ok != true {
		log.Fatal(err)
	}
	msg = []byte("test")
	...
	privKey, err := x509.ReadPrivateKeyFromPem("priv.pem", nil) // 读取密钥
	if err != nil {
		log.Fatal(err)
	}
	pubKey, err = x509.ReadPublicKeyFromPem("pub.pem", nil) // 读取公钥
	if err != nil {
		log.Fatal(err)
	}
	...
	_, err = x509.CreateCertificateRequestToPem("req.pem", &templateReq, privKey)
	if err != nil {
		log.Fatal(err)
	}
	req, err := x509.ReadCertificateRequestFromPem("req.pem")
	if err != nil {
		log.Fatal(err)
	}
	...
	template := x509.Certificate{
		...
		ExtraExtensions: []pkix.Extension{
			...
			{
				Id:       x509.oidExtensionSubjectKeyId,
				Critical: false,
	...
	pubKey, _ = priv.Public().(*PublicKey)
	ok, _ = x509.CreateCertificateToPem("cert.pem", &template, &template, pubKey, privKey)
	if ok != true {
		fmt.Printf("failed to create cert file\n")
	}
	cert, err := x509.ReadCertificateFromPem("cert.pem")
	if err != nil {
